...
class TextAnswer:
    ...
    def alternatives(self, threshold: float = 0.0):
        return [text for text, score in self.top_results[1:] if score >= threshold]
...
class AnswerIndex:
    ...
    def add_records(self, records):
        vector = np.array([record.embedding for record in records], dtype=np.float32)
    ...
    def column(self, key):
        if key not in self._columns:
            self._columns[key] = np.array([record.metadata.get(key) for record in self.records], dtype=object)
    ...
    def search(self, query, num_results=3):
        ...
        num_results = min( num_results, int(np.isfinite(scores).sum()))
        if num_results == 0:
            return []

        top = np.argpartition(-scores,  num_results - 1)[:num_results]
...
class AnswerRanker:
    ...
    def predict(self, question, visual_prediction, k=3):
        ...
        ranked = self.index.search(query, num_results=k)

        if not ranked:
            Logger.warning("no candidates matched")
            return None
        top_results = [(record.text, score) for record, score in ranked]
        return TextAnswer(top_results[0][0], top_results[0][1], top_results)
